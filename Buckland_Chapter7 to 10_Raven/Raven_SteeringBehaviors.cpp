...
Vector2D Raven_Steering::OffsetPursuit(const Raven_Bot* agent, const Vector2D offset)
{
	//calculate the offset's position in world space
	Vector2D WorldOffsetPos = PointToWorldSpace(offset,
		agent->Heading(),
		agent->Side(),
		agent->Pos().operator-=(Vector2D(20, 20)));
	...
	double LookAheadTime = ToOffset.Length() /
		(m_pRaven_Bot->MaxSpeed() + agent->Speed());

	//now Arrive at the predicted future position of the offset
	return Arrive(WorldOffsetPos + agent->Velocity() * LookAheadTime, fast);
}
